...
namespace episteme {
Position::Position() : position_history{}, state{} { position_history.reserve(1024); }
...
void Position::from_startpos() {
    ...
    auto setup_rank = [&](int rank, Color color) {
        state.mailbox[sq_idx(static_cast<Square>(rank * 8 + 0))] =
            piece_type_with_color(PieceType::Rook, color);
        state.mailbox[sq_idx(static_cast<Square>(rank * 8 + 1))] =
            piece_type_with_color(PieceType::Knight, color);
        state.mailbox[sq_idx(static_cast<Square>(rank * 8 + 2))] =
            piece_type_with_color(PieceType::Bishop, color);
        state.mailbox[sq_idx(static_cast<Square>(rank * 8 + 3))] =
            piece_type_with_color(PieceType::Queen, color);
        state.mailbox[sq_idx(static_cast<Square>(rank * 8 + 4))] =
            piece_type_with_color(PieceType::King, color);
        state.mailbox[sq_idx(static_cast<Square>(rank * 8 + 5))] =
            piece_type_with_color(PieceType::Bishop, color);
        state.mailbox[sq_idx(static_cast<Square>(rank * 8 + 6))] =
            piece_type_with_color(PieceType::Knight, color);
        state.mailbox[sq_idx(static_cast<Square>(rank * 8 + 7))] =
            piece_type_with_color(PieceType::Rook, color);
    };
...
void Position::make_move(const Move& move) {
    ...
    switch (move.move_type()) {
        ...
        case MoveType::Castling: {
            ...
            Square rook_src = king_side ? state.allowed_castles.rooks[us].kingside
                                        : state.allowed_castles.rooks[us].queenside;
            Square rook_dst = (side == Color::White) ? (king_side ? Square::F1 : Square::D1)
...
bool Position::is_insufficient() {
    ...
    if (state.bitboards[piece_type_idx(PieceType::Queen)] |
        state.bitboards[piece_type_idx(PieceType::Rook)])
        return false;
    if ((state.bitboards[piece_type_idx(PieceType::Bishop)] &
         state.bitboards[color_idx(Color::White) + COLOR_OFFSET]) &&
        (state.bitboards[piece_type_idx(PieceType::Bishop)] &
         state.bitboards[color_idx(Color::Black) + COLOR_OFFSET]))
        return false;
    if (state.bitboards[piece_type_idx(PieceType::Bishop)] &&
        state.bitboards[piece_type_idx(PieceType::Knight)])
        return false;
...
std::string Position::to_FEN() const {
    ...
    fen += (state.ep_square == Square::None
                ? "-"
                : Move(state.ep_square, state.ep_square).to_string().substr(2)) +
           " ";
...
Move from_UCI(const Position& position, const std::string& move) {
    ...
    if (is_castling())
        ...
        return Move(src, dst, MoveType::Promotion, char2Piece(move.at(4)));
    else
        return Move(src, dst);
}
}  // namespace episteme
