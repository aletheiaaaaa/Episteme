...
public:
    ...
    [[nodiscard]] inline uint64_t piece_bb(PieceType piece_type, Color color) const {
        return (
            state.bitboards[piece_type_idx(piece_type)] &
            state.bitboards[color_idx(color) + COLOR_OFFSET]
        );
    }
    ...
    [[nodiscard]] inline std::array<uint64_t, 8> bitboards_all() const { return state.bitboards; }

    [[nodiscard]] inline uint64_t bitboard(int index) const { return state.bitboards[index]; }

    [[nodiscard]] inline Color STM() const { return static_cast<Color>(state.stm); }

    [[nodiscard]] inline Color NTM() const { return static_cast<Color>(!state.stm); }

    [[nodiscard]] inline uint8_t half_move_clock() const { return state.half_move_clock; }

    [[nodiscard]] inline uint32_t full_move_number() const { return state.full_move_number; }

    [[nodiscard]] inline Square ep_square() const { return state.ep_square; }

    [[nodiscard]] inline AllowedCastles all_rights() const { return state.allowed_castles; }
    ...
    [[nodiscard]] inline Piece mailbox(int index) const { return state.mailbox[index]; }

    [[nodiscard]] inline std::array<Piece, 64> mailbox_all() const { return state.mailbox; }

    [[nodiscard]] inline uint64_t hash() const { return state.hash; }

    [[nodiscard]] inline uint64_t pawn_hash() const { return state.pawn_hash; }
