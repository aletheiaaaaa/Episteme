...
class Position {
  ...
  uint64_t piece_bb(PieceType piece_type, Color color) const {
    return (
  ...
  uint64_t piece_type_bb(PieceType piece_type) const {
    return current.bitboards[piece_type_idx(piece_type)];
  }

  uint64_t color_bb(Color color) const {
    return current.bitboards[color_idx(color) + COLOR_OFFSET];
  }

  std::array<uint64_t, 8> bitboards_all() const { return current.bitboards; }

  uint64_t bitboard(int index) const { return current.bitboards[index]; }

  Color STM() const { return static_cast<Color>(current.stm); }

  Color NTM() const { return static_cast<Color>(!current.stm); }

  uint8_t half_move_clock() const { return current.half_move_clock; }

  uint32_t full_move_number() const { return current.full_move_number; }

  Square ep_square() const { return current.ep_square; }

  AllowedCastles all_rights() const { return current.allowed_castles; }

  AllowedCastles::RookPair castling_rights(Color stm) const {
  ...
  std::array<Piece, 64> mailbox_all() const { return current.mailbox; }

  uint64_t full_hash() const { return current.hashes.full_hash; }

  uint64_t pawn_hash() const { return current.hashes.pawn_hash; }

  uint64_t major_hash() const { return current.hashes.major_hash; }

  uint64_t minor_hash() const { return current.hashes.minor_hash; }

  uint64_t non_pawn_stm_hash() const {
    return (!current.stm) ? current.hashes.non_pawn_white_hash : current.hashes.non_pawn_black_hash;
  }

  uint64_t non_pawn_ntm_hash() const {
