...
std::unique_ptr<const NNUE, decltype(nnue_deleter)>
    nnue(reinterpret_cast<const NNUE *>(gNNUEData), nnue_deleter);

Accumulator update(const Position &position, const Move &move,
                   Accumulator accum) {
...
bool SEE(const Position &position, const Move &move, int32_t threshold) {
  ...
  uint64_t pawn_threats = ((get_pawn_sq_attacks(to_sq, Color::White) &
  ...
  uint64_t all_threats = (pawn_threats | knight_threats | bishop_threats |
                          rook_threats | queen_threats | king_threats);
  ...
  while (true) {
    ...
    if ((next_threat = our_threats & pawn_bb)) {
      ...
      all_threats |= get_bishop_attacks_direct(to_sq, occupied_bb) &
                     (bishop_bb | queen_bb);
