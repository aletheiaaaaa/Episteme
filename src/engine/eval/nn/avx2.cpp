...
int32_t NNUE::l1_forward(const Accumulator& accum, Color stm) const {
  ...
  for (int i = 0; i < L1_WIDTH; i += 32) {
    auto partial = [&](__m256i temp, int offset) {
      ...
      __m256i l1w0 =
        _mm256_load_si256(reinterpret_cast<const __m256i*>(&l1_weights[i + offset + 0 * L1_WIDTH]));
      __m256i l1w1 =
        _mm256_load_si256(reinterpret_cast<const __m256i*>(&l1_weights[i + offset + 1 * L1_WIDTH]));

      stm_ =
        _mm256_min_epi16(_mm256_max_epi16(stm_, _mm256_setzero_si256()), _mm256_set1_epi16(QA));
      ntm_ =
        _mm256_min_epi16(_mm256_max_epi16(ntm_, _mm256_setzero_si256()), _mm256_set1_epi16(QA));
