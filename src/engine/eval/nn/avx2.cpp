  ...
  for (int i = 0; i < 2; i++) {
    ...
    for (int j = 0; j < L1_WIDTH / 2; j += 64) {
      auto process_chunk = [&](int offset) {
        __m256i acc00 = _mm256_loadu_si256(
          reinterpret_cast<const __m256i*>(&accum[j + offset + 0 + 0 * (L1_WIDTH / 2)])
        );
        __m256i acc01 = _mm256_loadu_si256(
          reinterpret_cast<const __m256i*>(&accum[j + offset + 0 + 1 * (L1_WIDTH / 2)])
        );
        __m256i acc10 = _mm256_loadu_si256(
          reinterpret_cast<const __m256i*>(&accum[j + offset + 16 + 0 * (L1_WIDTH / 2)])
        );
        __m256i acc11 = _mm256_loadu_si256(
          reinterpret_cast<const __m256i*>(&accum[j + offset + 16 + 1 * (L1_WIDTH / 2)])
        );

        acc00 =
          _mm256_min_epi16(_mm256_max_epi16(acc00, _mm256_setzero_si256()), _mm256_set1_epi16(QA));
        acc10 =
          _mm256_min_epi16(_mm256_max_epi16(acc10, _mm256_setzero_si256()), _mm256_set1_epi16(QA));
        ...
        _mm256_storeu_si256(
          reinterpret_cast<__m256i*>(&out[j + offset + is_ntm * (L1_WIDTH / 2)]), pair
        );

        uint8_t nnz =
          _mm256_movemask_ps(_mm256_castsi256_ps(_mm256_cmpgt_epi32(pair, _mm256_setzero_si256())));
        const auto& nnz_indices = _mm_loadu_si128(reinterpret_cast<const __m128i*>(&table[nnz]));

        _mm_storeu_si128(
          reinterpret_cast<__m128i*>(&indices[count]), _mm_add_epi16(base, nnz_indices)
        );
  ...
  for (int i = 0; i < L2_WIDTH; i += BLOCK_HEIGHT) {
    ...
    for (; j < count - count % 4; j += 4) {
      ...
      __m256i w0 = _mm256_loadu_si256(
        reinterpret_cast<const __m256i*>(&l1_weights[i / BLOCK_HEIGHT][indices[j + 0]])
      );
      __m256i w1 = _mm256_loadu_si256(
        reinterpret_cast<const __m256i*>(&l1_weights[i / BLOCK_HEIGHT][indices[j + 1]])
      );
      __m256i w2 = _mm256_loadu_si256(
        reinterpret_cast<const __m256i*>(&l1_weights[i / BLOCK_HEIGHT][indices[j + 2]])
      );
      __m256i w3 = _mm256_loadu_si256(
        reinterpret_cast<const __m256i*>(&l1_weights[i / BLOCK_HEIGHT][indices[j + 3]])
      );

      acc0 = _mm256_add_epi32(
        acc0, _mm256_madd_epi16(_mm256_maddubs_epi16(x0, w0), _mm256_set1_epi16(1))
      );
      acc1 = _mm256_add_epi32(
        acc1, _mm256_madd_epi16(_mm256_maddubs_epi16(x1, w1), _mm256_set1_epi16(1))
      );
      acc2 = _mm256_add_epi32(
        acc2, _mm256_madd_epi16(_mm256_maddubs_epi16(x2, w2), _mm256_set1_epi16(1))
      );
      acc3 = _mm256_add_epi32(
        acc3, _mm256_madd_epi16(_mm256_maddubs_epi16(x3, w3), _mm256_set1_epi16(1))
      );
    }
    ...
    for (; j < count; ++j) {
      ...
      __m256i w = _mm256_loadu_si256(
        reinterpret_cast<const __m256i*>(&l1_weights[i / BLOCK_HEIGHT][indices[j]])
      );

      acc0 =
        _mm256_add_epi32(acc0, _mm256_madd_epi16(_mm256_maddubs_epi16(x, w), _mm256_set1_epi16(1)));
    }

    __m256i acc = _mm256_add_epi32(_mm256_add_epi32(_mm256_add_epi32(acc0, acc1), acc2), acc3);
    __m256 b = _mm256_loadu_ps(&l1_biases[i]);

    __m256 pre = _mm256_fmadd_ps(
      _mm256_cvtepi32_ps(acc), _mm256_set1_ps(float(1 << SHIFT) / float(QA * QA * QB)), b
    );
