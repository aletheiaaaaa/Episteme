...
int32_t NNUE::l1_forward(const Accumulator& accum, Color stm) const {
  ...
  for (int i = 0; i < L1_WIDTH; i += 128) {
    auto partial = [&](__m512i temp, int offset) {
      ...
      __m512i l1w0 =
        _mm512_load_si512(reinterpret_cast<const __m512i*>(&l1_weights[i + offset + 0 * L1_WIDTH]));
      __m512i l1w1 =
        _mm512_load_si512(reinterpret_cast<const __m512i*>(&l1_weights[i + offset + 1 * L1_WIDTH]));

      stm_ =
        _mm512_min_epi16(_mm512_max_epi16(stm_, _mm512_setzero_si512()), _mm512_set1_epi16(QA));
      ntm_ =
        _mm512_min_epi16(_mm512_max_epi16(ntm_, _mm512_setzero_si512()), _mm512_set1_epi16(QA));
