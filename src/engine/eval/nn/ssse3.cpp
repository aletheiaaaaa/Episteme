...
L0Output NNUE::l0_pairwise(const Accumulator& accum, Color stm, L1Indices& indices) const {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    for (int j = 0; j < L1_WIDTH / 2; j += 16) {
      __m128i acc00 =
        _mm_loadu_si128(reinterpret_cast<const __m128i*>(&accum[j + 0 + 0 * (L1_WIDTH / 2)]));
      __m128i acc01 =
        _mm_loadu_si128(reinterpret_cast<const __m128i*>(&accum[j + 0 + 1 * (L1_WIDTH / 2)]));
      __m128i acc10 =
        _mm_loadu_si128(reinterpret_cast<const __m128i*>(&accum[j + 8 + 0 * (L1_WIDTH / 2)]));
      __m128i acc11 =
        _mm_loadu_si128(reinterpret_cast<const __m128i*>(&accum[j + 8 + 1 * (L1_WIDTH / 2)]));
  ...
  for (int i = 0; i < L2_WIDTH; i += BLOCK_HEIGHT) {
    ...
    for (; j < count - count % 2; j += 2) {
      ...
      __m128i w0 = _mm_loadu_si128(
        reinterpret_cast<const __m128i*>(&l1_weights[i / BLOCK_HEIGHT][indices[j + 0]])
      );
      __m128i w1 = _mm_loadu_si128(
        reinterpret_cast<const __m128i*>(&l1_weights[i / BLOCK_HEIGHT][indices[j + 1]])
      );
    ...
    for (; j < count; ++j) {
      ...
      __m128i w = _mm_loadu_si128(
        reinterpret_cast<const __m128i*>(&l1_weights[i / BLOCK_HEIGHT][indices[j]])
      );
    ...
    __m128 pre = _mm_add_ps(
      _mm_mul_ps(_mm_cvtepi32_ps(acc), _mm_set1_ps(float(1 << SHIFT) / float(QA * QA * QB))), b
    );
