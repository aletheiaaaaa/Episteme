...
public:
        ...
        auto get_hist = [&](int16_t diff) {
            ...
            if (ply > diff - 1) {
                ...
                if (prev_piece != Piece::None)
                    ply_value += cont_hist[piece_idx(piece)][sq_idx(move.to_square())]
    ...
    int32_t get_capt_hist(Piece attacker, Move move, Piece victim) {
        return capt_hist[piece_idx(attacker)][sq_idx(move.to_square())][piece_type_idx(victim)]
            .value;
    }
    ...
    void update_cont_hist(stack::Stack& stack, Piece piece, Move move, int16_t bonus, int16_t ply) {
        auto update_hist = [&](int16_t diff) {
            if (ply > diff - 1) {
                ...
                if (prev_piece != Piece::None)
                    cont_hist[piece_idx(piece)][sq_idx(move.to_square())][piece_idx(prev_piece)]
                             [sq_idx(prev_move.to_square())]
    ...
    void update_corr_hist(
        ...
        non_pawn_stm_corr_hist[color_idx(stm)][position.non_pawn_stm_hash() % 16384].update(
            correction, MAX_CORR_HIST
        );
        non_pawn_ntm_corr_hist[color_idx(stm)][position.non_pawn_ntm_hash() % 16384].update(
            correction, MAX_CORR_HIST
        );
