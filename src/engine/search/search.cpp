...
int32_t Worker::search(
    ...
    for (size_t i = 0; i < move_list.count; i++) {
        ...
        if (ply > 0 && best > -MATE + MAX_SEARCH_PLY) {
            ...
            if (!is_PV && is_quiet &&
                history.get_hist(
                    stack, from_pc, to_pc, move, position.STM(), ply, position.pawn_hash()
                ) <= history_margin)
                continue;
        }
        ...
        int16_t extension = 0;
        if (ply > 0 && depth >= 8 && move.data() == tt_entry.move.data() &&
            !stack[ply].excluded.data() && tt_entry.depth >= depth - 3 &&
            ...
            int32_t score = search<false>(
                position, PV, new_depth, ply, new_beta - 1, new_beta, cut_node, limits
            );
            stack[ply].excluded = Move();
        ...
        if (num_legal >= 4 && depth >= 3) {
            ...
            score = -search<false>(
                position, candidate, reduced, ply + 1, -alpha - 1, -alpha, true, limits
            );
        ...
        if (is_PV && (num_legal == 1 || score > alpha)) {
            score = -search<true>(
                position, candidate, new_depth, ply + 1, -beta, -alpha, false, limits
            );
        }
...
void Engine::run(Position& position) {
    ...
    for (int depth = 1; depth <= max_depth; depth++) {
        ...
        int32_t display_score =
            is_mate ? ((1 + MATE - std::abs(report.score)) / 2) * ((report.score > 0) ? 1 : -1)
                    : report.score;

        std::cout << "info depth " << report.depth << " time " << report.time << " nodes "
                  << report.nodes << " nps " << report.nps << " score "
