    ...
    int32_t Worker::search(Position& position, Line& PV, int16_t depth, int16_t ply, int32_t alpha, int32_t beta, bool cut_node) {
        ...
        for (size_t i = 0; i < move_list.count; i++) { 
            ...
            if (ply > 0 && best > -MATE + MAX_SEARCH_PLY) {
                ...
                const int32_t see_threshold = (is_quiet) ? quiet_see_base() + quiet_see_mult() * depth : noisy_see_base() + noisy_see_mult() * depth * depth + history.get_capt_hist(from_pc, move, to_pc) / 64;
