...
int32_t Worker::eval_correction(const Position& position) {
  ...
  correction +=
    240 * history.get_non_pawn_stm_corr_hist(position.non_pawn_stm_hash(), position.STM());
  correction +=
    240 * history.get_non_pawn_ntm_corr_hist(position.non_pawn_ntm_hash(), position.STM());
...
ScoredMove Worker::score_move(
  ...
  if (is_capture) {
    ...
    scored_move.score += history.get_capt_hist(
      src,
      move,
...
int32_t Worker::search(
  ...
  if (!stack[ply].excluded && !in_check(position, position.STM())) {
    if (!is_PV && depth <= 5 && static_eval >= beta + std::max(depth - improving, 0) * 100)
      return static_eval;
    ...
    if (!is_PV && depth >= 3) {
      ...
      if (no_pawns_or_kings) {
        ...
        int32_t score = -search<false>(
          position, null, depth - reduction, ply + 1, -beta, -beta + 1, !cut_node, limits
        );
        position.unmake_move();
  ...
  for (size_t i = 0; i < move_list.count; i++) {
    ...
    if (
      ...
      int32_t score =
        search<false>(position, PV, new_depth, ply, new_beta - 1, new_beta, cut_node, limits);
      stack[ply].excluded = Move();

      if (should_stop) return 0;

      if (score < new_beta)
        extension = (!is_PV && score < new_beta - 50) ? 2 : 1;
    ...
    if (num_legal >= 4 && depth >= 3) {
      ...
      score =
        -search<false>(position, candidate, reduced, ply + 1, -alpha - 1, -alpha, true, limits);
      if (score > alpha && reduced < depth - 1) {
        score = -search<false>(
          position, candidate, new_depth, ply + 1, -alpha - 1, -alpha, !cut_node, limits
        );
      }
    ...
    if (score > alpha) {
      ...
      if (score >= beta) {
        ...
        for (size_t j = 0; j < explored_noisies.count; j++) {
          ...
          Piece prev_from_pc = position.mailbox(prev_move.from_square());
          Piece prev_to_pc = move.move_type() == MoveType::EnPassant
                               ? piece_type_with_color(PieceType::Pawn, position.NTM())
...
int32_t Worker::quiesce(
  Position& position, Line& PV, int16_t ply, int32_t alpha, int32_t beta, SearchLimits limits
) {
  if (nodes % 2000 == 0 && limits.time_exceeded()) {
    should_stop = true;
    return 0;
...
Report Worker::run(
  ...
  Report report{
    .depth = params.depth, .time = elapsed, .nodes = nodes, .nps = nps, .score = score, .line = PV
  };
...
void Engine::run(Position& position) {
  ...
  for (int depth = 1; depth <= max_depth; depth++) {
    ...
    std::cout << "info depth " << report.depth << " time " << report.time << " nodes "
              << report.nodes << " nps " << report.nps << " score " << (is_mate ? "mate " : "cp ")
              << display_score << " pv ";
