...
void game_loop(const Parameters& params, std::ostream& stream, uint32_t id) {
    ...
    for (int i = 0; i < workload && !stop; i++) {
        ...
        while (!stop) {
            ...
            if (!scored_move.move) {
                wdl = in_check(position, position.STM()) ? (position.STM() == Color::Black ? 2 : 0)
                                                         : 1;
                break;
            } else {
                if (std::abs(scored_move.score) >= search::MATE - search::MAX_SEARCH_PLY)
                    ...
                    if (win_plies >= WIN_PLIES_MIN)
                        wdl = 2;
                    else if (loss_plies >= WIN_PLIES_MIN)
                        wdl = 0;
                    else if (draw_plies >= DRAW_PLIES_MIN)
                        wdl = 1;
                }
            }
...
void run(Parameters& params) {
    ...
    if (stop) std::cout << "Datagen interrupted; saving partial data.\n";
    ...
    for (const auto& file : files) {
        ...
        if (input) {
            ...
            if (std::remove(file.c_str()) == 0)
                ...
                std::cout << "Error removing file " << file << " after concatenation\n";
        } else {
