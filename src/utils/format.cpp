...
void Format::push(Move move, int32_t score) {
  ...
  viri_move |= (move.from_idx() | (to_idx << 6) | (move.promo_idx() << 12) |
                (move.type_idx() << 14));
...
size_t Format::write(std::ostream &stream, uint8_t wdl) {
  ...
  stream.write(reinterpret_cast<const char *>(moves.data()),
               sizeof(ScoredMove) * moves.size());
  stream.write(reinterpret_cast<const char *>(NULL_TERMINATOR.data()),
               sizeof(ScoredMove));
