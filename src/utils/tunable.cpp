...
namespace episteme::tunable {
    ...
    void init_lmr_table() {
        for (int i = 1; i < 64; i++) {
            for (int j = 1; j < 64; j++) {
                lmr_table_noisy[i][j] = lmr_table_noisy_base() + std::log(i) * std::log(j) / lmr_table_noisy_div();
                lmr_table_quiet[i][j] = lmr_table_quiet_base() + std::log(i) * std::log(j) / lmr_table_quiet_div();
            }
        }
    }
#if ENABLE_TUNING
